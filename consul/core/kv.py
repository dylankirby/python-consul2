class KV(object):
    ...
    def get(
        ...
        return self.agent.http.get(
            CB.json(index=True, decode=decode, one=one, map=lambda x: x if x else None),
            path="/v1/kv/%s" % key,
            params=params,
            headers=headers,
        )
    ...
    def put(
        ...
        ), value is None or isinstance(value, (six.string_types, six.binary_type))
    ...
    def delete(self, key, recurse=None, cas=None, token=None, dc=None):
        ...
        return self.agent.http.delete(
            CB.json(), path="/v1/kv/%s" % key, params=params, headers=headers
        )
